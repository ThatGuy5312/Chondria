...
// this was from my old game engine. its ChatGPT because i dont want to make all of this and i couldn't find a already made one.
public class ImGuiController : IDisposable
{
    ...
    {
        ImGui.GetIO().MousePos = new(e.X, e.Y);
    }
...
void main()
{
    ...
    private void RenderImDrawData(ImDrawDataPtr drawData)
    {
        ...
        for (int n = 0; n < drawData.CmdListsCount; n++)
        {
            ...
            GL.BindBuffer(BufferTarget.ArrayBuffer, _vertexBuffer);
            GL.BufferData(BufferTarget.ArrayBuffer, cmdList.VtxBuffer.Size * vertexSize, cmdList.VtxBuffer.Data, BufferUsageHint.StreamDraw);

            int indexSize = sizeof(ushort);
            GL.BindBuffer(BufferTarget.ElementArrayBuffer, _indexBuffer);
            GL.BufferData(BufferTarget.ElementArrayBuffer, cmdList.IdxBuffer.Size * indexSize, cmdList.IdxBuffer.Data, BufferUsageHint.StreamDraw);
            ...
            for (int cmdi = 0; cmdi < cmdList.CmdBuffer.Size; cmdi++)
            {
                ...
                var io = ImGui.GetIO();
                GL.Scissor(
                    (int)(pcmd.ClipRect.X * io.DisplayFramebufferScale.X),
                    (int)((io.DisplaySize.Y - pcmd.ClipRect.W) * io.DisplayFramebufferScale.Y),
                    (int)((pcmd.ClipRect.Z - pcmd.ClipRect.X) * io.DisplayFramebufferScale.X),
                    (int)((pcmd.ClipRect.W - pcmd.ClipRect.Y) * io.DisplayFramebufferScale.Y)
                );
